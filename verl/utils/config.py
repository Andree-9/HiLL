...
def validate_config(
    ...
    # Account for hinter expansion: train_batch_size * num_hints * n
    effective_train_batch_size = config.data.train_batch_size
    if config.get("use_hinter", False) and hasattr(config, "hinter"):
        num_hints = config.hinter.get("num_hints", 1)
        n_rollouts = config.actor_rollout_ref.rollout.get("n", 1)
        effective_train_batch_size = config.data.train_batch_size * num_hints * n_rollouts
