...
class ResourcePoolManager:
    ...
    # Optional: per-pool colocate count override.
    # Needed when running reasoner and hinter as separate worker groups
    # on the same GPU pool.
    max_colocate_count_spec: dict[str, int] = field(default_factory=dict)
    ...
    def create_resource_pool(self):
        ...
        for resource_pool_name, process_on_nodes in self.resource_pool_spec.items():
            ...
            max_colocate_count = self.max_colocate_count_spec.get(resource_pool_name, 1)
...
def compute_prompt_statistics(
    ...
    """Compute prompt-level all-correct/all-incorrect/zero-variance ratios."""
    if num_repeat <= 1:
        return {
            "zero_adv/prompts_all_correct_ratio": 0.0,
            "zero_adv/prompts_all_incorrect_ratio": 0.0,
            "zero_adv/prompts_zero_advantage_ratio": 0.0,
        }, [], [], []
    ...
    for i, uid in enumerate(uid_index):
        uid_to_rewards[uid].append(sequence_rewards[i].item())

    num_prompts = len(uid_to_rewards)
    prompts_all_correct = 0
    prompts_all_incorrect = 0
    prompts_zero_advantage = 0
    zero_variance_uids = []
    all_correct_uids = []
    all_incorrect_uids = []

    for uid, rewards in uid_to_rewards.items():
        if len(rewards) < num_repeat:
            continue

        is_all_correct = all(r > positive_threshold for r in rewards)
        if is_all_correct:
            prompts_all_correct += 1
            all_correct_uids.append(uid)

        is_all_incorrect = all(r <= positive_threshold for r in rewards)
        if is_all_incorrect:
            prompts_all_incorrect += 1
            all_incorrect_uids.append(uid)

        if len(set(rewards)) == 1 or (max(rewards) - min(rewards)) < 1e-6:
            prompts_zero_advantage += 1
            zero_variance_uids.append(uid)

    stats = {
        "zero_adv/prompts_all_correct_ratio": prompts_all_correct / num_prompts if num_prompts > 0 else 0.0,
        "zero_adv/prompts_all_incorrect_ratio": prompts_all_incorrect / num_prompts if num_prompts > 0 else 0.0,
        "zero_adv/prompts_zero_advantage_ratio": prompts_zero_advantage / num_prompts if num_prompts > 0 else 0.0,
    }
    return stats, zero_variance_uids, all_correct_uids, all_incorrect_uids
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # collect metrics
                prompt_stats, _, _, _ = compute_prompt_statistics(
                    data=batch,
                    num_repeat=self.config.actor_rollout_ref.rollout.n,
                )
                metrics.update(prompt_stats)
