...
class ScriptArguments:
    ...
    disable_thinking: Optional[bool] = field(
        default=False,
        metadata={"help": "Pass enable_thinking=False to apply_chat_template (for Qwen3 models)"},
    )
...
def main():
    ...
    if script_args.disable_thinking:
        chat_template_kwargs["enable_thinking"] = False

    def make_prompt(example):
        question = example["problem"]
        prompt_messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": question}]
        return {"prompt": tokenizer.apply_chat_template(prompt_messages, tokenize=False, add_generation_prompt=True, **chat_template_kwargs)}
